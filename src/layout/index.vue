<template>
  <div class="app-shell">
    ...
    <UiDialog v-model="settingsOpen" title="连接你的聊天档案" eyebrow="YOUR DATA, YOUR CONTROL"><form id="connection-form" @submit.prevent="saveConnection"><label class="field"><span>chatlog 服务地址</span><input v-model="endpoint" placeholder="http://127.0.0.1:5030" autocomplete="off" spellcheck="false"/><p>留空使用同源代理。连接远程服务时，请仅填写你信任的地址。</p></label><div class="notice"><UiIcon name="shield" :size="17"/><p>仅在浏览器中保存服务地址和外观设置，不保存聊天正文。HTTPS 网页连接 HTTP 服务可能被浏览器拦截。</p></div><p v-if="settingsError" class="error-text" role="alert">{{ settingsError }}</p><p v-if="demoEnabled" class="error-text">演示模式不会发送连接测试请求。退出演示后再连接真实数据。</p></form><template #footer><button class="btn" @click="settingsOpen=false">取消</button><button class="btn btn-primary" type="submit" form="connection-form" :disabled="demoEnabled">保存并重新连接</button></template></UiDialog>
    <UiDialog v-model="searchOpen" title="找回一段对话" eyebrow="SEARCH YOUR ARCHIVE"><form id="global-search-form" @submit.prevent="submitSearch"><label class="field"><span>搜索聊天内容</span><input ref="globalInput" v-model="globalKeyword" placeholder="输入你记得的一句话、一个关键词…" autofocus/><p>将跳转到聊天记录，使用后端关键词查询。</p></label></form><template #footer><button class="btn" @click="searchOpen=false">取消</button><button class="btn btn-primary" type="submit" form="global-search-form" :disabled="!globalKeyword.trim()"><UiIcon name="search" :size="15"/>搜索</button></template></UiDialog>
...
export default {name:'ArchiveLayout',components:{UiIcon,BrandMark,UiAvatar,UiDialog},setup(){
  ...
  const navigation=[{label:'工作空间',items:[{path:'/dashboard',label:'总览',icon:'grid'},{path:'/analytics',label:'数据分析',icon:'chart'}]},{label:'我的档案',items:[{path:'/chatlog',label:'聊天记录',icon:'chat'},{path:'/contacts',label:'联系人',icon:'user'},{path:'/chatrooms',label:'群聊',icon:'users'},{path:'/sessions',label:'会话',icon:'inbox'},{path:'/media',label:'媒体库',icon:'image'}]}]
  const currentTitle=computed(()=>navigation.flatMap(g=>g.items).find(item=>item.path===route.path)?.label || '总览')
  const connectionLabel=computed(()=>demoEnabled?'演示数据':workspace.loading?'正在连接':!workspace.ready?'尚未连接':Object.keys(workspace.errors).length===0?'已连接':Object.keys(workspace.errors).length===3?'连接失败':'部分可用')
  ...
  const saveConnection=async()=>{try{setApiBase(endpoint.value);resetWorkspace();settingsOpen.value=false;await loadWorkspace();notify(Object.keys(workspace.errors).length?'部分数据未能读取，请查看页面提示。':'服务已连接')}catch(error){settingsError.value=error.message}}
