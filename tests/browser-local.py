"""Exercise the actual production WASM worker using fictional SQLite fixtures."""
...
class Handler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if not Path(self.translate_path(self.path.split('?')[0])).exists():
...
try:
  with tempfile.TemporaryDirectory() as temporary, sync_playwright() as p:
    ...
    expect(page.locator('.source-metrics strong').first).to_have_text(str(fixture['messages']),timeout=90000)
    ...
    assert messages['total']==60
    assert len({item['id'] for item in messages['data']})==60
    ...
    result=rpc(page,'chatlog',{'keyword':'春风','limit':20})
    ...
    first=rpc(page,'chatlog',{'limit':20,'offset':0})
    second=rpc(page,'chatlog',{'limit':20,'offset':20})
    ...
    for route in ['dashboard','chatlog','analytics','contacts','chatrooms','sessions','media']:
      page.goto('http://127.0.0.1:8780/'+route)
      page.wait_for_timeout(1200)
      assert page.locator('body').evaluate('(el)=>el.scrollWidth<=window.innerWidth+1')
      assert not page.locator('.state-panel.error').count()
      if route in ['chatlog','analytics','media']:
    ...
    checked('chat UI performs full-index keyword search without HTML execution')
    page.get_by_role('button',name='导出记录',exact=True).click()
    with page.expect_download() as download:
      page.get_by_role('button',name='下载文件',exact=True).click()
    path=download.value.path()
    assert '春风' in Path(path).read_text(encoding='utf-8-sig')
    ...
    requests.clear()
    page.reload();expect(page.locator('.local-media-card')).to_have_count(4,timeout=30000)
    assert not any('/api/v1/' in url or method!='GET' for method,url in requests)
    checked('archive and attachment persistence after reload with no backend requests or uploads')
    page.goto('http://127.0.0.1:8780/sources')
